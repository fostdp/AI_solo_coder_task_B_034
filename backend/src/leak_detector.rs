...
mod tests {
    ...
    fn test_fft_basic_sine_wave() {
        ...
        let (detector, _) = LeakDetector::new(config);
    ...
    fn test_calculate_spectral_features_empty_signal() {
        ...
        let result = detector.calculate_spectral_features(&data, None, 0.0, 0.0);
    ...
    async fn test_detect_leaks_with_leak() {
        let config = create_test_config();
        let (detector, _rx) = LeakDetector::new(config.clone());
    ...
    async fn test_detect_leaks_no_leak() {
        let config = create_test_config();
        let (detector, _rx) = LeakDetector::new(config);
    ...
    fn test_generate_leak_animations() {
        ...
        let animations = LeakDetector::generate_leak_animations(&[leak.clone()]);
    ...
    fn test_generate_leak_animations_empty() {
        let animations = LeakDetector::generate_leak_animations(&[]);
    ...
    #[test]
    fn test_backward_compatibility_hydrogen_leak_detector_type() {
        use crate::hydrogen_leak_detector::HydrogenLeakDetector;

        let config = create_test_config();
        let (detector, _rx) = LeakDetector::new(config.clone());
        let (legacy, _rx2) = HydrogenLeakDetector::new(config);

        assert_eq!(
            std::mem::size_of::<LeakDetector>(),
            std::mem::size_of::<HydrogenLeakDetector>()
        );
    }

    #[test]
    fn test_backward_compatibility_leak_request_type() {
        use crate::hydrogen_leak_detector::LeakDetectionRequest as LegacyRequest;
        use crate::leak_detector::LeakDetectionRequest as NewRequest;
