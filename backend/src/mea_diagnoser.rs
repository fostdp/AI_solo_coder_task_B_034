...
mod tests {
    ...
    fn generate_test_eis_data(params: &EquivalentCircuitParams, noise: f64) -> Vec<EISDataPoint> {
        ...
        let engine = MEADiagnoser::new(create_test_config()).0;
    ...
    fn test_equivalent_circuit_fit_normal_case() {
        ...
        let (engine, _) = MEADiagnoser::new(config);
    ...
    async fn test_full_diagnosis_pipeline() {
        let config = create_test_config();
        let (engine, _rx) = MEADiagnoser::new(config);
    ...
    fn test_temperature_compensation_reference_temperature() {
        let config = create_test_config();
        let (engine, _) = MEADiagnoser::new(config.clone());
    ...
    #[test]
    fn test_backward_compatibility_mea_diagnostics_type() {
        use crate::mea_diagnostics::MEADiagnostics;
        use crate::mea_diagnostics::MEADiagnosticEngine;

        let config = create_test_config();
        let (diagnoser, _rx) = MEADiagnoser::new(config.clone());
        let (engine, _rx2) = MEADiagnosticEngine::new(config.clone());
        let (legacy, _rx3) = MEADiagnostics::new(config);

        assert_eq!(
            std::mem::size_of::<MEADiagnoser>(),
            std::mem::size_of::<MEADiagnosticEngine>()
        );
        assert_eq!(
            std::mem::size_of::<MEADiagnoser>(),
            std::mem::size_of::<MEADiagnostics>()
        );
    }

    #[test]
    fn test_backward_compatibility_request_type() {
        use crate::mea_diagnostics::MEADiagnosticRequest as LegacyRequest;
        use crate::mea_diagnoser::MEADiagnosticRequest as NewRequest;

        assert_eq!(
            std::mem::size_of::<LegacyRequest>(),
            std::mem::size_of::<NewRequest>()
        );
    }
}
